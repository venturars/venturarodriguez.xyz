...
interface SwapQuoteJson extends Omit<
  QuotePrice,
...
/**
 * Fetches a firm swap quote from the backend API and normalizes numeric fields to bigint.
 *
 * @param params - Quote request values including chain, tokens and sell amount.
 * @returns Parsed quote payload with bigint values for amounts, fees and tx fields.
...
export async function getQuote(
  ...
  const url = `${import.meta.env.PUBLIC_API_URL}/swaps/get-quote?${searchParams.toString()}`;
  ...
  const payload = (await response.json()) as SwapQuoteJson;
  return {
    ...payload,
    tokenIn: {
      ...payload.tokenIn,
